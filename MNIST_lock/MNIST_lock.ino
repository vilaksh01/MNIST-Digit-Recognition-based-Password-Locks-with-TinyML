...
#define SHUTDOWN_BUTTON_X1              250
#define SHUTDOWN_BUTTON_Y1             190
#define SHUTDOWN_BUTTON_X2               SHUTDOWN_BUTTON_X1 + 60
#define SHUTDOWN_BUTTON_Y2             SHUTDOWN_BUTTON_Y1 + 40
#define ENTER_BUTTON_X1              250
#define ENTER_BUTTON_Y1             140
#define ENTER_BUTTON_X2               ENTER_BUTTON_X1 + 60
#define ENTER_BUTTON_Y2             ENTER_BUTTON_Y1 + 40
...
void SetPIN()
{
  while (M5.Touch.ispressed())
  {
    ...
    else if ((coordinate.x > ENTER_BUTTON_X1 && coordinate.x < ENTER_BUTTON_X2) && (coordinate.y > ENTER_BUTTON_Y1 && coordinate.y < ENTER_BUTTON_Y2 ))
    {
...
void SetPIN_Menu()
{
  ...
  M5.Lcd.drawString("SET PIN", 245, 65, 2);

  M5.Lcd.setTextColor(BLACK);
  M5.Lcd.setTextDatum(ML_DATUM);    //Set text alignment to center-aligned
  if (devicePIN == "") M5.Lcd.drawString("****", 245, 95, 4);
  else M5.Lcd.drawString(devicePIN, 245, 95, 4);

  M5.Lcd.fillRoundRect(250, 140, 60, 40, 10, RED);   // Predict button box

  M5.Lcd.setTextColor(WHITE);
  M5.Lcd.setTextDatum(ML_DATUM);    //Set text alignment to center-aligned
  M5.Lcd.drawString("ENTER", 260, 160, 2);
}
...
void Unlock_Menu()
{
  ...
  M5.Lcd.setTextColor(BLACK);
  M5.Lcd.setTextDatum(ML_DATUM);    //Set text alignment to center-aligned
  M5.Lcd.drawString("Entered PIN", 245, 65, 2);


  M5.Lcd.setTextColor(BLACK);
  M5.Lcd.setTextDatum(ML_DATUM);    //Set text alignment to center-aligned
  if (enteredPIN == "")  M5.Lcd.drawString("****", 245, 95, 4);
  else M5.Lcd.drawString(enteredPIN, 245, 95, 4);
  

  M5.Lcd.setTextColor(WHITE);
  M5.Lcd.setTextDatum(ML_DATUM);    //Set text alignment to center-aligned
  M5.Lcd.drawString("ENTER", 255, 160, 2);

  M5.Lcd.setTextColor(BLACK);
  M5.Lcd.setTextDatum(ML_DATUM);    //Set text alignment to center-aligned
  M5.Lcd.drawString("SHUTDOWN", 250, 210, 2);
...
void setup()
{
  ...
  M5.Lcd.drawString("PIN " + devicePIN + " set successful", 20, 80, 4);
...
void loop()
{
  ...
  if (M5.Touch.ispressed())
  {
    ...
    else if ((coordinate.x > ENTER_BUTTON_X1 && coordinate.x < ENTER_BUTTON_X2) && (coordinate.y > ENTER_BUTTON_Y1 && coordinate.y < ENTER_BUTTON_Y2 ) && c < 4)
    {
      for (int i = 0; i < INPUT_IMAGE_WIDTH; ++i)
      {
        for (int j = 0; j < INPUT_IMAGE_HEIGHT;  ++j)
    ...
    else if ((coordinate.x > SHUTDOWN_BUTTON_X1 && coordinate.x < SHUTDOWN_BUTTON_X2) && (coordinate.y > SHUTDOWN_BUTTON_Y1 && coordinate.y < SHUTDOWN_BUTTON_Y2 ))
    {
      M5.shutdown();
    }
    ...
    else if (c > 3)
    {
      ...
      if (enteredPIN.equals(devicePIN))
      {
        M5.Lcd.setTextColor(GREEN);
        M5.Lcd.drawString("Device Unlocked :)", 20, 80, 4);
      }
      else
      {
        M5.Lcd.setTextColor(RED);
        M5.Lcd.drawString("Wrong PIN :(", 20, 80, 4);
      }
      c = 0;
    }
